...
function PendingSessionCard({ item, onResume }: { item: CaptureSessionSummary; onResume: (sessionId: string) => void }) {
  ...
  return <button onClick={() => onResume(item.id)} className="w-full rounded-2xl border-2 border-ink bg-lime-wash p-3 text-left shadow-[3px_3px_0_0_#1c2b0a]">
    <span className="block text-xs font-bold text-leaf">{isTranscribing ? "正在转写" : STATUS_LABEL[item.status]}</span>
    <span className="mt-1 block line-clamp-2 font-bold text-ink">{isTranscribing ? "录音已保存，正在生成文字" : item.marker_transcript_preview}</span>
