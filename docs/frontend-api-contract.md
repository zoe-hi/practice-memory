...
文字标记创建成功后保存响应 `id` 并进入会话详情。语音标记在用户再次点击结束录音后
立即提交；收到 `201` 即保存 `id` 并跳转“我的”待处理列表，不等待后台转写，也不再显示
“活动完成，去整理”确认按钮。创建成功并不代表 `marker_transcript` 已立刻可用；列表中
`status=marked` 且 `marker_transcript_preview=null` 时应显示“正在转写”。用户进入该记录并
开始复盘时，后端会对仍缺少转写的初始音频同步重试。
...
前端提供“文字描述 / 语音描述”切换。语音由浏览器 `MediaRecorder` 录制，停止后由用户
明确提交本次一次性查询；录音超过 15 MiB 时在前端拦截。真实音频转写需要后端启用
可用的 ASR Provider，`FakeAIProvider` 不猜测未知音频内容。
