...
5. 当 question_count 大于等于 2 时应立即返回最终草稿；仅在仍有关键冲突或信息缺口时允许再问最后一个问题。question_count 大于等于 3 时必须返回最终草稿，绝不能继续提问。
...
class DashScopeAIProvider(AIProvider):
    ...
    def advance_reflection(
        ...
        def parse(response: Any) -> ReflectionAdvanceResult:
            ...
            if question_count >= 3 and not result.ready_for_confirmation:
                raise ProviderInvalidOutputError(
